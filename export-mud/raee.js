/**
 * Esportazione delle schede RAEE MUD
 * @module raee
 * @memberof module:mud
...
/** Esporta la riga relativa al record RA – Schede TRA-RAEE / CR-RAEE – Trattamento / Centro di Raccolta
 ...
 * @param {string} tipoScheda Tipo di SCHEDA ("TRA" oppure "CRR")
 * @param {number} categoriaRAEE Categoria RAEE “Valore ammesso compreso tra 1 e 10”
 * @param {boolean} isCategSuddiviseMedianteStima Suddivisione in categorie effettuata mediante stima
 * @param {number} nRT Numero di moduli RT-RAEE allegati (Valorizzato sia per Tipo di Scheda = "TRA" che per Tipo di Scheda = “CRR")
 * @param {decimal} kgConsTerzi Quantità di rifiuto consegnato a terzi
 * @param {number} nDR Numero di moduli DR-RAEE allegati
 * @param {number} nTE Numero di moduli TE-RAEE allegati
 * @param {boolean} isRecSmaltPressoUL Attività di recupero o smaltimento svolte presso l'unità locale (Valorizzare solo per Tipo di Scheda = “CRR”)
 * @param {number} nMG Numero di moduli MG-RAEE allegati
 * @param {decimal} kgGiac Rifiuti in giacenza al 31/12
 * @param {decimal} kgSmalt Quantità a smaltimento (Valorizzare solo per Tipo di Scheda = “TRA”)
 * @param {decimal} kgRecMat Quantità a recupero di materia (Valorizzare solo per Tipo di Scheda = “TRA”)
 * @param {decimal} kgRecEnerg Quantità a recupero energia (Valorizzare solo per Tipo di Scheda = “TRA”)
 * @param {decimal} kgRiutAppIntera RAEE riutilizzati come apparecchiatura intera (Valorizzare solo per Tipo di Scheda = “TRA”)
 * @param {utils.recordExportCallback} [callback] - Funzione di callback
 * @return {string} (Se non gestito callback) Record RA – Schede TRA-RAEE / CR-RAEE – Trattamento / Centro di Raccolta
 */
exports.getRecordRAEE = function(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                                  tipoScheda, categoriaRAEE, isCategSuddiviseMedianteStima,
                                  nRT, kgConsTerzi, nDR, nTE, isRecSmaltPressoUL, nMG, kgGiac,
                                  kgSmalt, kgRecMat, kgRecEnerg, kgRiutAppIntera,
                                  callback)
{
    var objConsTerzi = utils.getOggettoValoreUM(kgConsTerzi);
    var objGiac = utils.getOggettoValoreUM(kgGiac);
    var objSmalt = utils.getOggettoValoreUM(kgSmalt);
    var objRecMat = utils.getOggettoValoreUM(kgRecMat);
    var objRecEnerg = utils.getOggettoValoreUM(kgRecEnerg);
    var objRiutAppIntera = utils.getOggettoValoreUM(kgRiutAppIntera);
    var retVal = util.format(
        ...
        utils.formattaBoolean(isCategSuddiviseMedianteStima),
        ...
        utils.formattaBoolean(isRecSmaltPressoUL),
        utils.formattaNumero(nMG, 5),
        utils.formattaNumero(objGiac.valore, 7, 3),
        utils.formattaNumero(objGiac.um, 1),
        utils.formattaNumero(objSmalt.valore, 7, 3),
        utils.formattaNumero(objSmalt.um, 1),
        utils.formattaNumero(objRecMat.valore, 7, 3),
        utils.formattaNumero(objRecMat.um, 1),
        utils.formattaNumero(objRecEnerg.valore, 7, 3),
        utils.formattaNumero(objRecEnerg.um, 1),
        utils.formattaNumero(objRiutAppIntera.valore, 7, 3),
        utils.formattaNumero(objRiutAppIntera.um, 1)
    );
    if(callback)
        return callback(retVal.length != 160, retVal);
    else
        return retVal;
}

/** Esporta la riga relativa al record RB – Schede TRA-RAEE / CR-RAEE - Dettaglio dei Codici CER Raccolti, Ricevuti, Prodotti
 * @param {string} annoRiferimentoDichiarazione Anno di riferimento della dichiarazione (AAAA)
 * @param {string} codiceFiscale Codice fiscale identificativo
 * @param {string} codUL Codice di identificazione univoca dell’unità locale
 * @param {string} tipoScheda Tipo di SCHEDA ("TRA" oppure "CRR")
 ...
 * @param {string} tipoRifiuto Tipo di Rifiuto (DR=Domestico Raccolto/Ricevuto,PR=Professionale Raccolto/Ricevuto, PP=Prodotto nell'unità locale) Da valorizzare solo per Tipo di Scheda = "TRA". Lasciare vuoto per Tipo di Scheda = “CRR”
 * @param {decimal} kgQta Quantità
 * @param {utils.recordExportCallback} [callback] - Funzione di callback
 * @return {string} (Se non gestito callback) Record RB – Schede TRA-RAEE / CR-RAEE - Dettaglio dei Codici CER Raccolti, Ricevuti, Prodotti
 */
exports.getRecordRAEE_Det = function(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                                      tipoScheda, categoriaRAEE,
                                      codiceCER,
                                      tipoRifiuto, kgQta,
                                      callback)
{
    ...
    if(callback)
        return callback(retVal.length != 72, retVal);
    else
        return retVal;
}

/** Esporta la riga relativa al record RC – Moduli RT-RAEE – Rifiuto ricevuto da terzi
 * @param {string} annoRiferimentoDichiarazione Anno di riferimento della dichiarazione (AAAA)
 * @param {string} codiceFiscale Codice fiscale identificativo
 * @param {string} codUL Codice di identificazione univoca dell’unità locale
 * @param {string} tipoScheda Tipo di SCHEDA ("TRA" oppure "CRR")
 ...
 * @param {boolean} isSoggettoPrivato Soggetto che ha conferito il rifiuto: Privati
 * @param {utils.recordExportCallback} [callback] - Funzione di callback
 * @return {string} (Se non gestito callback) Record RC – Moduli RT-RAEE – Rifiuto ricevuto da terzi
 */
exports.getRecordRAEE_RT = function(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                                     tipoScheda, categoriaRAEE,
                                     codiceCER,
                                     nProgressivoAllegato,
                                     codiceFiscaleSoggetto, ragioneSociale, istatProvincia, istatComune, via, civico, cap,
                                     kgQta, nazioneEstera, codRegolamentoCEE_1013_2006, isSoggettoPrivato,
                                     callback)
{
    return getRecordRAEE_RT_TE(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                               tipoScheda, categoriaRAEE,
                               codiceCER,
...
/** Esporta la riga relativa al record RC – Moduli TE-RAEE – Elenco dei Vettori Utilizzati
 * @param {string} annoRiferimentoDichiarazione Anno di riferimento della dichiarazione (AAAA)
 * @param {string} codiceFiscale Codice fiscale identificativo
 * @param {string} codUL Codice di identificazione univoca dell’unità locale
 * @param {string} tipoScheda Tipo di SCHEDA ("TRA" oppure "CRR")
 * @param {number} categoriaRAEE Categoria RAEE “Valore ammesso compreso tra 1 e 10”
 * @param {string} codiceCER Codice del rifiuto SOLO CATALOGO EUROPEO
 * @param {number} nProgressivoAllegato Numero progressivo dell'allegato “TE-RAEE”
 ...
 * @param {utils.recordExportCallback} [callback] - Funzione di callback
 * @return {string} (Se non gestito callback) Record RC – Moduli TE-RAEE – Elenco dei Vettori Utilizzati
 */
exports.getRecordRAEE_TE = function(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                                     tipoScheda, categoriaRAEE,
                                     codiceCER,
                                     nProgressivoAllegato,
...
/** Esporta la riga generica relativa al record RC – Moduli RT-RAEE / TE-RAEE – Rifiuto ricevuto da terzi / Elenco dei Vettori Utilizzati
 * @param {string} annoRiferimentoDichiarazione Anno di riferimento della dichiarazione (AAAA)
 * @param {string} codiceFiscale Codice fiscale identificativo
 * @param {string} codUL Codice di identificazione univoca dell’unità locale
 * @param {string} tipoScheda Tipo di SCHEDA ("TRA" oppure "CRR")
 ...
 * @param {number} nProgressivoAllegato Numero progressivo dell'allegato “RT-RAEE” / “TE-RAEE”
 ...
 * @param {string} ragioneSociale Nome o Ragione sociale (Valorizzare solo per Privati=NO)
 * @param {number} istatProvincia ISTAT Provincia (Valorizzare solo per modulo RT-RAEE e Privati=NO)
 * @param {number} istatComune ISTAT Comune (Valorizzare solo per modulo RT-RAEE e Privati=NO)
 * @param {string} via Via (Valorizzare solo per modulo RT-RAEE e Privati=NO)
 * @param {string} civico Nr. Civico (Valorizzare solo per modulo RT-RAEE e Privati=NO)
 * @param {string} cap CAP (Valorizzare solo per modulo RT-RAEE e Privati=NO)
 * @param {decimal} kgQta Quantità dichiarata (Ricevuta per modulo RT-RAEE, Trasportata per modulo TE-RAEE)
 * @param {string} nazioneEstera Nome della nazione (solo se trattasi di paese Estero) (Valorizzare solo per modulo RT-RAEE)
 * @param {string} codRegolamentoCEE_1013_2006 Codice Regolamento CEE 1013/2006 (2 lettere + 4 cifre) (Valorizzare solo per modulo RT-RAEE)
 * @param {boolean} isSoggettoPrivato Soggetto che ha conferito il rifiuto: Privati (Valorizzare solo per modulo RT-RAEE)
 * @param {utils.recordExportCallback} [callback] - Funzione di callback
 * @return {string} (Se non gestito callback) Record RC – Moduli RT-RAEE / TE-RAEE – Rifiuto ricevuto da terzi / Elenco dei Vettori Utilizzati
 */
function getRecordRAEE_RT_TE(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                              tipoScheda, categoriaRAEE,
                              codiceCER,
                              ...
                              callback)
{
    ...
    if(callback)
        return callback(retVal.length != 238, retVal);
    else
        return retVal;
}


/** Esporta la riga relativa al record RD – Modulo DR-RAEE – Rifiuti conferiti a Terzi
 * @param {string} annoRiferimentoDichiarazione Anno di riferimento della dichiarazione (AAAA)
 * @param {string} codiceFiscale Codice fiscale identificativo
 * @param {string} codUL Codice di identificazione univoca dell’unità locale
 * @param {string} tipoScheda Tipo di SCHEDA ("TRA" oppure "CRR")
 * @param {number} categoriaRAEE Categoria RAEE “Valore ammesso compreso tra 1 e 10”
 * @param {string} codiceCER Codice del rifiuto SOLO CATALOGO EUROPEO
 * @param {number} nProgressivoAllegato Numero progressivo dell'allegato “DR-RAEE”
 ...
 * @param {utils.recordExportCallback} [callback] - Funzione di callback
 * @return {string} (Se non gestito callback) Record RD – Modulo DR-RAEE – Rifiuti conferiti a Terzi
 */
exports.getRecordRAEE_DR = function(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                                     tipoScheda, categoriaRAEE,
                                     codiceCER,
                                     nProgressivoAllegato,
                                     codiceFiscaleSoggetto, ragioneSociale, istatProvincia, istatComune, via, civico, cap,
                                     ...
                                     callback)
{
    ...
    if(callback)
        return callback(retVal.length != 469, retVal);
    else
        return retVal;
}


/** Esporta le righe relative ai record RE / RF – Moduli MG-RAEE – Gestione dei rifiuti, Operazioni di Smaltimento / Recupero
 * @param {string} annoRiferimentoDichiarazione Anno di riferimento della dichiarazione (AAAA)
 * @param {string} codiceFiscale Codice fiscale identificativo
 * @param {string} codUL Codice di identificazione univoca dell’unità locale
 * @param {string} tipoScheda Tipo di SCHEDA ("TRA" oppure "CRR")
 ...
 * @param {utils.recordExportCallback} [callback] - Funzione di callback
 * @return {string} (Se non gestito callback) Record RE / RF – Moduli MG-RAEE – Gestione dei rifiuti, Operazioni di Smaltimento / Recupero
 */
exports.getRecordRAEE_MG = function(annoRiferimentoDichiarazione, codiceFiscale, codUL,
                                     tipoScheda, categoriaRAEE,
                                     codiceCER,
                                     ...
                                     callback)
{
    ...
    if(callback)
        return callback(retVal.length != 245 + 2 + 271, retVal);
    else
        return retVal;
}
